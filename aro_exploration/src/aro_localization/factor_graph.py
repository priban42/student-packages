...
import time
import unittest
from typing import List, Optional, Tuple, Sequence, Dict, Any, Set
...
from aro_localization.utils import coords_to_transform_matrix, merge_dicts, transform_matrix_to_coords, nsec_to_sec, \
    normalize_angle_symmetric, angular_signed_distance, r2w, w2r, col_vector, row_vector, forward, atleast_2d_row
...
class FactorGraphState:
    ...
    def __init__(self):
        ...
        # Relative marker
        self.mr = np.array([[np.nan, np.nan, np.nan]]).transpose()  # Initialization with NaNs
...
class FactorGraphConfig:
    ...
    def __init__(self, ma=(12.0, 40.0, 0.0), mr_gt: Optional[np.ndarray] = None, fuse_icp=False,
                 solver_options: Dict[str, Any] = None, method_options: Dict[str, Any] = None,
                 icp_yaw_scale=1.0, marker_yaw_scale=1.0):
        """
        ...
        self.method_options = self.all_methods_options.get(self.solver_options["method"], {})
...
class FactorGraph:
    ...
    def __init__(self, ma=(12.0, 40.0, 0.0), mr_gt: Optional[np.ndarray] = None, fuse_icp=False,
                 solver_options: Optional[Dict[str, Any]] = None, method_options: Optional[Dict[str, Any]] = None,
                 icp_yaw_scale=1.0, marker_yaw_scale=1.0):
        """
        :param ma: Pose of the absolute marker in wcf (3-tuple x, y, yaw).
        :param mr_gt: Ground truth pose of the relative marker in wcf (3-tuple x, y, yaw).
        ...
        self.config = FactorGraphConfig(
            ma, mr_gt, fuse_icp, solver_options, method_options, icp_yaw_scale, marker_yaw_scale)
        self.state = FactorGraphState()

    @staticmethod
    def add_z(state: FactorGraphState, config: FactorGraphConfig,
              odom_samples: Dict[int, Tuple[np.ndarray, np.ndarray]],
              z_mr: Sequence[float], c_mr: np.ndarray,
              z_ma: Sequence[float], c_ma: np.ndarray,
              icp_samples: Dict[int, Tuple[np.ndarray, np.ndarray]],
              z_gt_odom: Sequence[float],
              stamp: Optional[int] = None) -> FactorGraphState:
        ...
        state.x_icp_odom = np.vstack((state.x_icp_odom, row_vector([np.nan] * state.x_icp_odom.shape[1])))
        for t in sorted(icp_samples.keys()):
            ...
            while np.any(np.isnan(state.x_icp_odom[last_icp_t, :])) and last_icp_t > 0:
                last_icp_t -= 1
            state.x_icp_odom[i + 1, :] = r2w(state.z_icp[:, i], state.x_icp_odom[last_icp_t, :]).ravel()

        return state

    @staticmethod
    def optimize(state: FactorGraphState, config: FactorGraphConfig) -> FactorGraphState:
        ...
        for i in range(state.last_optimized_idx - 1, x.shape[0] - 1):
            if np.any(np.isnan(x[i + 1, :])):
                ...
                z = z_icp[:, i] if i in idx_icp and not np.any(np.isnan(z_icp[:, i])) else z_odom[:, i]
        ...
        logdebug("Before opt: size: %i, x %s, mr %s, ma %s" % (
            state.last_optimized_idx, str(x[-1]), str(np.ravel(mr)), str(np.ravel(config.ma))))
        ...
        with contextlib.redirect_stdout(io.StringIO()) as opt_log:
            sol: OptimizeResult = least_squares(
                FactorGraph.compute_residuals_opt, x_ini, FactorGraph.compute_residuals_jacobian_opt,
                args=(state, config, idx_odom, idx_mr, idx_ma, idx_icp), **solver_options)
        ...
        x[:, 2] = normalize_angle_symmetric(x[:, 2])  # Wrap the computed yaw between -pi and pi
        mr[2, :] = normalize_angle_symmetric(mr[2, :])  # Wrap the computed marker yaw between -pi and pi
        ...
        logdebug("After opt: size %i, x %s, mr %s, ma %s" % (
            state.last_optimized_idx, str(x[-1]), str(np.ravel(mr)), str(np.ravel(config.ma))))
        duration = time.perf_counter() - tic
        logdebug("After opt: time %0.2f seconds, nfev %i, njev %i, "
                 "time/it %0.3f s, time/fev %0.3f s, time/fev/size %0.2f ms" % (
            duration, sol.nfev, sol.njev, duration / sol.njev, duration / sol.nfev,
    ...
    @staticmethod
    def res(x: np.ndarray, x2: np.ndarray, z: np.ndarray, cost: np.ndarray) -> np.ndarray:
    ...
    @staticmethod
    def res_jac(x: np.ndarray, x2: np.ndarray, z: np.ndarray, cost: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    @staticmethod
    def compute_residuals(state: FactorGraphState, config: FactorGraphConfig,
                          idx_odom: np.ndarray, idx_mr: np.ndarray, idx_ma: np.ndarray, idx_icp: np.ndarray) -> \
        ...
        x_next_odom = x[:, [i + 1 for i in idx_odom]]  # States following the states with odometry measurements
        res_odom = FactorGraph.res(x[:, idx_odom], x_next_odom, state.z_odom[:, idx_odom], state.c_odom[idx_odom, :, :])

        # Relative marker measurements residuals
        res_mr = FactorGraph.res(x[:, idx_mr], state.mr, state.z_mr[:, idx_mr], state.c_mr[idx_mr, :, :])
        res_mr[2, :] *= config.marker_yaw_scale

        # Absolute marker measurements residuals
        res_ma = FactorGraph.res(x[:, idx_ma], config.ma, state.z_ma[:, idx_ma], state.c_ma[idx_ma, :, :])
        res_ma[2, :] *= config.marker_yaw_scale

        res_icp = None
        if config.fuse_icp:
            # ICP odom measurements residuals
            x_next_icp = x[:, [i + 1 for i in idx_icp]]  # States following the states with ICP measurements
            res_icp = FactorGraph.res(x[:, idx_icp], x_next_icp, state.z_icp[:, idx_icp], state.c_icp[idx_icp, :, :])
            res_icp[2, :] *= config.icp_yaw_scale

        return res_odom, res_mr, res_ma, res_icp

    @staticmethod
    def compute_residuals_opt(xmr: np.ndarray, state: FactorGraphState, config: FactorGraphConfig,
                              idx_odom: np.ndarray, idx_mr: np.ndarray, idx_ma: np.ndarray, idx_icp: np.ndarray) -> \
            np.ndarray:
        """Like compute_residuals, but adapted shapes to work with the optimizer."""
        # unpack x, mr from the concatenated state
        state.x, state.mr = xmr[0:-3].reshape(int((xmr.shape[0] - 3) / 3), 3, order='C'), xmr[-3:].reshape(3, 1)

        res_odom, res_mr, res_ma, res_icp = FactorGraph.compute_residuals(
            state, config, idx_odom, idx_mr, idx_ma, idx_icp)
    ...
    def compute_residuals_jacobian(
            state: FactorGraphState, config: FactorGraphConfig,
            idx_odom: np.ndarray, idx_mr: np.ndarray, idx_ma: np.ndarray, idx_icp: np.ndarray) -> \
        ...
        x_next_odom = x[:, [i + 1 for i in idx_odom]]  # States following the states with odometry measurements
        J, J1 = FactorGraph.res_jac(x[:, idx_odom], x_next_odom, z_odom[:, idx_odom], c_odom[idx_odom, :, :])
        ...
        J, Jm = FactorGraph.res_jac(x[:, idx_mr], state.mr, z_mr[:, idx_mr], c_mr[idx_mr, :, :])
        J[2, :, :] *= config.marker_yaw_scale
        Jm[2, :, :] *= config.marker_yaw_scale
        J_mr = FactorGraph.build_sparse_jacobian(J, Jm, idx_mr, num_variables, is_mr=True)
        ...
        J, _ = FactorGraph.res_jac(x[:, idx_ma], config.ma, z_ma[:, idx_ma], c_ma[idx_ma, :, :])
        J[2, :, :] *= config.marker_yaw_scale
        ...
        x_next_icp = x[:, [i + 1 for i in idx_icp]]  # States following the states with ICP measurements
        J, J1 = FactorGraph.res_jac(x[:, idx_icp], x_next_icp, z_icp[:, idx_icp], c_icp[idx_icp, :, :])
    ...
    @staticmethod
    def build_sparse_jacobian(J: np.ndarray, J1: Optional[np.ndarray], idxs: np.ndarray,
                              num_variables: int, is_mr=False) -> csr_matrix:
        ...
        data = np.hstack((
            np.moveaxis(J, -1, 0).reshape((J.shape[1] * J.shape[2], J.shape[0])),
            np.moveaxis(J1, -1, 0).reshape((J1.shape[1] * J1.shape[2], J1.shape[0]))
        )) if J1 is not None else np.moveaxis(J, -1, 0).reshape((J.shape[1] * J.shape[2], J.shape[0]))

        indices = np.zeros((num_constraints, dim * factor_arity))
        indices[:, 0:dim] = np.repeat(np.repeat(idxs[np.newaxis, :] * dim, dim, axis=0).T, dim,
                                             axis=0) + np.arange(dim, dtype=np.uint32)
        if factor_arity > 1:
            if is_mr:
                indices[:, dim:(2 * dim)] = \
                    np.arange(num_variables - dim, num_variables, dtype=np.uint32)[np.newaxis, :]
            else:
                indices[:, dim:(2 * dim)] = indices[:, 0:dim] + dim

        indptr = np.arange(0, num_constraints * dim * factor_arity + 1, dim * factor_arity)

        return csr_matrix((data.ravel(), indices.ravel(), indptr), shape=(num_constraints, num_variables))

    @staticmethod
    def compute_residuals_jacobian_opt(
            xmr: np.ndarray, state: FactorGraphState, config: FactorGraphConfig,
            idx_odom: np.ndarray, idx_mr: np.ndarray, idx_ma: np.ndarray, idx_icp: np.ndarray) -> csr_matrix:
    ...
    def visu(state: FactorGraphState, config: FactorGraphConfig, only_main=False):
        ...
        idx_odom = np.where((~np.isnan(z_odom) & ~np.isnan(x[:, :-1]) & ~np.isnan(x[:, 1:])).all(axis=0))[0]
        idx_icp = np.where((~np.isnan(z_icp) & ~np.isnan(x[:, :-1]) & ~np.isnan(x[:, 1:]) & ~np.isnan(x_icp_odom[:, :-1])).all(axis=0))[0]
        ...
        for k, j in enumerate(idx_icp):
            main_ax.plot([x_icp_odom[0, j], z_icp_wcf[0, k]], [x_icp_odom[1, j], z_icp_wcf[1, k]], ':', color=(0.6, 0.4, 0.5), linewidth=2, mew=2)

        main_ax.plot(x_odom[0, idx_odom[idx_odom <= last_optimized_idx]], x_odom[1, idx_odom[idx_odom <= last_optimized_idx]], '-', color='y', linewidth=1, mew=2, label="Integrated odom")
        main_ax.plot(x_odom[0, idx_odom[idx_odom >= last_optimized_idx]], x_odom[1, idx_odom[idx_odom >= last_optimized_idx]], '-', color=(1.0, 1.0, 0.5), linewidth=1, mew=2)
        main_ax.plot(x_icp_odom[0, idx_icp[idx_icp <= last_optimized_idx]], x_icp_odom[1, idx_icp[idx_icp <= last_optimized_idx]], '-', color='g', linewidth=1, mew=2, label="Integrated ICP odom")
        main_ax.plot(x_icp_odom[0, idx_icp[idx_icp >= last_optimized_idx]], x_icp_odom[1, idx_icp[idx_icp >= last_optimized_idx]], '-', color=(0.5, 1.0, 0.5), linewidth=1, mew=2)
        main_ax.plot(x[0, idx_x[idx_x <= last_optimized_idx]], x[1, idx_x[idx_x <= last_optimized_idx]], '-', color='b', linewidth=2, mew=2, label="Fused odom")
        main_ax.plot(x[0, idx_x[idx_x >= last_optimized_idx]], x[1, idx_x[idx_x >= last_optimized_idx]], '-', color=(0.5, 0.5, 1.0), linewidth=2, mew=2)
        if len(idx_odom) > 0:
            main_ax.scatter(x_odom[0, idx_odom[-1]], x_odom[1, idx_odom[-1]], color='y', s=40)
        if len(idx_icp) > 0:
            main_ax.scatter(x_icp_odom[0, idx_icp[-1]], x_icp_odom[1, idx_icp[-1]], color='g', s=40)
        if len(idx_x) > 0:
        ...
        if config.mr_gt is not None:
            main_ax.plot(config.mr_gt[0, 0], config.mr_gt[1, 0], 's', color='y', linewidth=1, mew=6, label="Rel.m. GT")
        ...
        stamps = nsec_to_sec(np.array(state.stamps + [state.stamps[-1]]) - state.stamps[0])
        ...
        yaw_ax.plot(stamps[idx_odom[idx_odom <= last_optimized_idx]], x_odom[2, idx_odom[idx_odom <= last_optimized_idx]], '-', color='y', linewidth=1, mew=2, label="Integrated odom")
        yaw_ax.plot(stamps[idx_odom[idx_odom >= last_optimized_idx]], x_odom[2, idx_odom[idx_odom >= last_optimized_idx]], '-', color=(1.0, 1.0, 0.5), linewidth=1, mew=2)
        yaw_ax.plot(stamps[idx_icp[idx_icp <= last_optimized_idx]], x_icp_odom[2, idx_icp[idx_icp <= last_optimized_idx]], '-', color='g', linewidth=1, mew=2, label="Integrated ICP odom")
        yaw_ax.plot(stamps[idx_icp[idx_icp >= last_optimized_idx]], x_icp_odom[2, idx_icp[idx_icp >= last_optimized_idx]], '-', color=(0.5, 1.0, 0.5), linewidth=1, mew=2)
        yaw_ax.plot(stamps[idx_x[idx_x <= last_optimized_idx]], x[2, idx_x[idx_x <= last_optimized_idx]], '-', color='b', linewidth=1, mew=2, label="Fused odom")
        yaw_ax.plot(stamps[idx_x[idx_x >= last_optimized_idx]], x[2, idx_x[idx_x >= last_optimized_idx]], '-', color=(0.5, 0.5, 1.0), linewidth=1, mew=2)
        ...
        res_scaled_odom, res_scaled_mr, res_scaled_ma, res_scaled_icp = FactorGraph.compute_residuals(
            state, config, idx_odom, idx_mr, idx_ma, idx_icp)

        state.c_odom[:] = np.repeat(np.eye(3)[None, :, :], state.c_odom.shape[0], axis=0)
        ...
        res_ax.plot(stamps[idx_odom[idx_odom <= last_optimized_idx]], np.linalg.norm(res_odom, axis=0)[idx_odom <= last_optimized_idx], label="res_odom", color='k')
        res_ax.plot(stamps[idx_odom[idx_odom >= last_optimized_idx]], np.linalg.norm(res_odom, axis=0)[idx_odom >= last_optimized_idx], color=(0.5, 0.5, 0.5))
        ...
        res_scaled_ax.plot(stamps[idx_odom[idx_odom <= last_optimized_idx]], np.linalg.norm(res_scaled_odom, axis=0)[idx_odom <= last_optimized_idx], label="res_scaled_odom", color='k')
        res_scaled_ax.plot(stamps[idx_odom[idx_odom >= last_optimized_idx]], np.linalg.norm(res_scaled_odom, axis=0)[idx_odom >= last_optimized_idx], color=(0.5, 0.5, 0.5))
        res_scaled_ax.plot(stamps[idx_mr[idx_mr <= last_optimized_idx]], np.linalg.norm(res_scaled_mr, axis=0)[idx_mr <= last_optimized_idx], label="res_scaled_mr", color='r')
        res_scaled_ax.plot(stamps[idx_mr[idx_mr >= last_optimized_idx]], np.linalg.norm(res_scaled_mr, axis=0)[idx_mr >= last_optimized_idx], color=(1.0, 0.5, 0.5))
        res_scaled_ax.plot(stamps[idx_ma[idx_ma <= last_optimized_idx]], np.linalg.norm(res_scaled_ma, axis=0)[idx_ma <= last_optimized_idx], label="res_scaled_ma", color='g')
        res_scaled_ax.plot(stamps[idx_ma[idx_ma >= last_optimized_idx]], np.linalg.norm(res_scaled_ma, axis=0)[idx_ma >= last_optimized_idx], color=(0.5, 1.0, 0.5))
        if config.fuse_icp:
            res_scaled_ax.plot(stamps[idx_icp[idx_icp <= last_optimized_idx]], np.linalg.norm(res_scaled_icp, axis=0)[idx_icp <= last_optimized_idx], label="res_scaled_icp", color='b')
            res_scaled_ax.plot(stamps[idx_icp[idx_icp >= last_optimized_idx]], np.linalg.norm(res_scaled_icp, axis=0)[idx_icp >= last_optimized_idx], color=(0.5, 0.5, 1.0))
        ...
        if idx_gt.shape[0] > 0:
            ...
            gt_ax.plot(stamps[idx_x[idx_x <= last_optimized_idx]], errors[idx_x[idx_x <= last_optimized_idx]], label="FG localization error", color='k')
            gt_ax.plot(stamps[idx_x[idx_x >= last_optimized_idx]], errors[idx_x[idx_x >= last_optimized_idx]], color=(0.5, 0.5, 0.5))
            gt_ax.plot(stamps[idx_x[idx_x <= last_optimized_idx]], errors_x[idx_x[idx_x <= last_optimized_idx]], label="X error", color='r')
            gt_ax.plot(stamps[idx_x[idx_x >= last_optimized_idx]], errors_x[idx_x[idx_x >= last_optimized_idx]], color=(1.0, 0.5, 0.5))
            gt_ax.plot(stamps[idx_x[idx_x <= last_optimized_idx]], errors_y[idx_x[idx_x <= last_optimized_idx]], label="Y error", color='g')
            gt_ax.plot(stamps[idx_x[idx_x >= last_optimized_idx]], errors_y[idx_x[idx_x >= last_optimized_idx]], color=(0.5, 1.0, 0.5))
            gt_ax.plot(stamps[idx_x[idx_x <= last_optimized_idx]], errors_yaw[idx_x[idx_x <= last_optimized_idx]], label="Yaw error", color='b')
            gt_ax.plot(stamps[idx_x[idx_x >= last_optimized_idx]], errors_yaw[idx_x[idx_x >= last_optimized_idx]], color=(0.5, 0.5, 1.0))
            ...
            odom_and_gt_idxs = np.where((~np.isnan(x_odom2) & ~np.isnan(gt_odom)).all(axis=0))[0]
            if odom_and_gt_idxs.shape[0] > 0:
                odom_errors_x = x_odom2[0, odom_and_gt_idxs] - gt_odom[0, odom_and_gt_idxs]
                odom_errors_y = x_odom2[1, odom_and_gt_idxs] - gt_odom[1, odom_and_gt_idxs]
                odom_errors_yaw = angular_signed_distance(x_odom2[2, odom_and_gt_idxs], gt_odom[2, odom_and_gt_idxs])
                ...
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], odom_errors[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], label="Odom localization error", color='m')
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], odom_errors[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], color=(0.5, 1.0, 1.0))
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], odom_errors_x[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], label="X error", color='r')
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], odom_errors_x[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], color=(1.0, 0.5, 0.5))
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], odom_errors_y[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], label="Y error", color='g')
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], odom_errors_y[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], color=(0.5, 1.0, 0.5))
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], odom_errors_yaw[odom_and_gt_idxs[odom_and_gt_idxs <= last_optimized_idx]], label="Yaw error", color='b')
                gt_odom_ax.plot(stamps[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], odom_errors_yaw[odom_and_gt_idxs[odom_and_gt_idxs >= last_optimized_idx]], color=(0.5, 0.5, 1.0))
...
class TestFactorGraph(unittest.TestCase):
    ...
    def test_res(self):
        ...
        res = FactorGraph.res(state.x[:-1, :].T, state.x[1:, :].T, state.z_odom, state.c_odom)

        self.assertSequenceEqual(res.shape, (3, 2))
        self.assertSequenceEqual(res.ravel().tolist(), np.zeros((3, 2)).ravel().tolist())

    def test_res_jac(self):
        config, state, idx_odom, idx_mr, idx_ma, idx_icp = self.generate_test_data()

        J1, J2 = FactorGraph.res_jac(state.x[:-1, :].T, state.x[1:, :].T, state.z_odom, state.c_odom)

        self.assertSequenceEqual(J1.shape, (3, 3, 2))
        self.assertSequenceEqual(J2.shape, (3, 3, 2))
        self.assertSequenceEqual(J1[:, :, 0].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J2[:, :, 0].ravel().tolist(), np.eye(3).ravel().tolist())
        self.assertSequenceEqual(J1[:, :, 1].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J2[:, :, 1].ravel().tolist(), np.eye(3).ravel().tolist())
    ...
    def test_compute_residuals(self):
        ...
        self.assertSequenceEqual(res_odom.ravel().tolist(), np.zeros((3, 2)).ravel().tolist())
        self.assertSequenceEqual(res_mr.ravel().tolist(), np.zeros((3, 2)).ravel().tolist())
        self.assertSequenceEqual(res_ma.ravel().tolist(), np.zeros((3, 1)).ravel().tolist())
        self.assertSequenceEqual(res_icp.ravel().tolist(), np.zeros((3, 2)).ravel().tolist())
    ...
    def test_compute_residuals_jacobian(self):
        ...
        self.assertSequenceEqual(J_odom[:3, :3].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J_odom[3:, :3].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_odom[:3, 3:6].ravel().tolist(), np.eye(3).ravel().tolist())
        self.assertSequenceEqual(J_odom[3:, 3:6].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J_odom[:3, 6:9].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_odom[3:, 6:9].ravel().tolist(), np.eye(3).ravel().tolist())
        self.assertSequenceEqual(J_odom[:3, 9:12].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_odom[3:, 9:12].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())

        self.assertSequenceEqual(J_mr[:3, :3].ravel().tolist(), [-1, 0, 0, 0, -1, -2, 0, 0, -1])
        self.assertSequenceEqual(J_mr[3:, :3].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_mr[:3, 3:6].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_mr[3:, 3:6].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J_mr[:3, 6:9].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_mr[3:, 6:9].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_mr[:3, 9:12].ravel().tolist(), np.eye(3).ravel().tolist())
        self.assertSequenceEqual(J_mr[3:, 9:12].ravel().tolist(), np.eye(3).ravel().tolist())

        self.assertSequenceEqual(J_ma[:3, :3].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J_ma[:3, 3:6].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_ma[:3, 6:9].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_ma[:3, 9:12].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())

        self.assertSequenceEqual(J_icp[:3, :3].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J_icp[3:, :3].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_icp[:3, 3:6].ravel().tolist(), np.eye(3).ravel().tolist())
        self.assertSequenceEqual(J_icp[3:, 3:6].ravel().tolist(), [-1, 0, 0, 0, -1, -1, 0, 0, -1])
        self.assertSequenceEqual(J_icp[:3, 6:9].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_icp[3:, 6:9].ravel().tolist(), np.eye(3).ravel().tolist())
        self.assertSequenceEqual(J_icp[:3, 9:12].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
        self.assertSequenceEqual(J_icp[3:, 9:12].ravel().tolist(), np.zeros((3, 3)).ravel().tolist())
...
def sim(trajectory_length=10, v=0.3, w=0.1,
        x0=np.array([0.0, 0.0, 0.0]), mr=np.array([[2.0], [1.0], [0.0]]), ma=np.array([[1.0], [0.0], [0.0]]),
        noise_odom=0.1, noise_mr=0.1, noise_ma=0.1, noise_icp=0.1):
...
def factor_graph_demo():
    ...
    x0, X, Mr, Ma, Z_odom, Z_ma, Z_mr, Z_icp = sim(
        trajectory_length=30, v=0.3, w=0.3, noise_odom=0.1, noise_mr=0.1, noise_ma=0.1, noise_icp=0.02)

    # Set costs for the measurements
    C_odom = 0.5 * np.repeat(np.eye(3)[None, :, :], Z_odom.shape[1], axis=0)
    C_mr = 1.0 * np.repeat(np.eye(3)[None, :, :], Z_mr.shape[1], axis=0)
    C_ma = 1.0 * np.repeat(np.eye(3)[None, :, :], Z_ma.shape[1], axis=0)
    C_icp = 2.0 * np.repeat(np.eye(3)[None, :, :], Z_icp.shape[1], axis=0)

    fuse_icp = False
    fg = FactorGraph(ma=Ma.ravel().tolist(), mr_gt=Mr.ravel().tolist(), fuse_icp=fuse_icp)
    state = fg.state
    ...
    for i in range(X.shape[1] - 1):
        ...
        z_odom = Z_odom[:, i]
        z_icp = Z_icp[:, i]
        z_mr = Z_mr[:, i] if i % mr_keep_every_nth == 0 else np.array([np.nan, np.nan, np.nan])
        z_ma = Z_ma[:, i] if i % ma_keep_every_nth == 0 else np.array([np.nan, np.nan, np.nan])
        t = int((i + 1) * 1e9)
        ...
        state = FactorGraph.add_z(
            state, fg.config, odom_samples, z_mr, C_mr[i, :, :], z_ma, C_ma[i, :, :], icp_samples, gt, t)
